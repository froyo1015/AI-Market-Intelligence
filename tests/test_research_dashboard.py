...
def test_research_shell():
    ...
    assert 'Scores are selection scores, not probabilities.' in page
    assert 'Not used for AI decisions' in page
    assert 'Production disabled' in page
...
def test_missing_references_visible():
    ...
    assert 'References unavailable' in out
...
""")
    assert 'Derivatives unavailable' in out
