...
Current DXY daily change and US10Y daily basis-point change are both positive. Attention level is medium and the condition is linked to current evidence.

後續觀察：Monitor the referenced observations for DXY, US10Y on the next validated run.
資料狀態：`current` · `validated`
引用：`top_655a677ae2c0ac5b`（完整來源見下方證據與引用）

### 2. Observed: BTC-USD and ETH-USD daily co-movement

The validated relationship condition is observed across BTC-USD, ETH-USD; BTC-USD daily_change_pct=1.0494 percent, ETH-USD daily_change_pct=0.7981 percent.

後續觀察：Monitor the same validated metrics and their observation timestamps on the next run.
資料狀態：`current` · `validated`
引用：`top_cc358c6f269ad2fe`（完整來源見下方證據與引用）


## 市場環境

市場環境暫無法分類：合資格證據不足；各維度與原因保留於下方 Regime Dimensions。
引用：`int_regime_b07490496322cfc9`

## 未來 24–48 小時風險

輸入沒有已驗證的近期事件風險；不代表未來沒有風險，資料覆蓋限制見下方。
- Observed dollar and yield pressure：Current DXY daily change and US10Y daily basis-point change are both positive.
  狀態：`observed`；引用：`int_risk_f57a2041b380aadc`

## 資料狀態

報告：`partial`；新鮮度：`stale`。
